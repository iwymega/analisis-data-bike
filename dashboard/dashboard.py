...
def create_daily_registered_rent_df(df):
    daily_registered_rent_df = (
        df.groupby(by="dateday").agg({"registered": "sum"}).reset_index()
    )
...
def create_season_rent_df(df):
    season_rent_df = (
        df.groupby(by="season")[["registered", "casual"]].sum().reset_index()
    )
...
def create_weekday_rent_df(df):
    weekday_rent_df = df.groupby(by="weekday").agg({"count": "sum"}).reset_index()
    return weekday_rent_df


# Menyiapkan workingday_rent_df
def create_workingday_rent_df(df):
    workingday_rent_df = df.groupby(by="workingday").agg({"count": "sum"}).reset_index()
    return workingday_rent_df


# Menyiapkan holiday_rent_df
def create_holiday_rent_df(df):
    holiday_rent_df = df.groupby(by="holiday").agg({"count": "sum"}).reset_index()
    return holiday_rent_df


# Menyiapkan weather_rent_df
def create_weather_rent_df(df):
    weather_rent_df = df.groupby(by="weather_cond").agg({"count": "sum"})
...
sns.barplot(
    x="workingday", y="count", data=workingday_rent_df, palette=colors1, ax=axes[0]
)
...
st.caption("Copyright (c) Megantara 2023")
